...
const BubbleCanvas: React.FC<BubbleCanvasProps> = ({
    ...
    const createBubbleProgressively = useCallback(
        (w: number, h: number) => {
            ...
            const shouldBeProfile = userProfiles.length > 0 &&
    ...
    const handleResize = useCallback(() => {
        ...
        if (!oldDimensions || isSignificantResize(w, h, oldDimensions.width, oldDimensions.height, isMobileRef.current)) {
    ...
    useEffect(() => {
        ...
        const animate = (currentTime: number) => {
            ...
            activeBubbles.forEach((bubble) =>
                bubble.draw(ctx, { width: w, height: h })
            );
